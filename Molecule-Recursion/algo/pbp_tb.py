...
class PBP_TB(GFNAlgorithm):
    ...
    def compute_batch_losses(
        ...
        if self.cfg.variant == TBVariant.SubTB1:
            ...
            if self.cfg.cum_subtb:
                traj_losses = self.subtb_cum(log_p_F, log_p_B.detach(), per_graph_out[:, 0], clip_log_R, batch.traj_lens)
            else:
                traj_losses = self.subtb_loss_fast(log_p_F, log_p_B.detach(), per_graph_out[:, 0], clip_log_R, batch.traj_lens)
            ...
            transition_losses = (F_sn + log_p_F - F_sm - log_p_B.detach()).pow(2)
            ...
            denominator = clip_log_R + traj_log_p_B.detach()
    ...
    def compute_batch_losses_for_FPB(
        self, model: TrajectoryBalanceModel, batch: gd.Batch, log_p, num_bootstrap: int = 0  
    ):
        dev = batch.x.device
        # A single trajectory is comprised of many graphs
        num_trajs = int(batch.traj_lens.shape[0])
