...
# Move two directories up to reach the parent directory of gflownet
...
class SEHTask(GFNTask):
    ...
    def compute_reward_from_graph(self, graphs: List[Data]) -> Tensor:
        ...
        preds[preds.isnan()] = 0
        return self.flat_reward_transform(preds).clip(1e-2, 100).reshape((-1,))
...
def main():
    ...
    config.cond.temperature.dist_params = [64.0, 64.0]
